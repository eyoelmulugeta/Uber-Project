...
result <- data.frame(
  ...
  Day = rep(1:31, times = 12),
  Week = rep(1:5, each = 7, length.out = 31),
  Total = sample(1:100, size = 12 * 31, replace = TRUE)
)
...
ui <- fluidPage(
  ...
  tabsetPanel(
    tabPanel("Month and Day", 
             sidebarLayout(
               sidebarPanel(
                 sliderInput("month_select", "Select Month", min = 1, max = 12, value = c(1, 12), step = 1),
                 sliderInput("day_select", "Select Day", min = 1, max = 31, value = c(1, 31), step = 1)
               ),
               mainPanel(
                 plotOutput("plot_month_day")
               )
             )
    ),
    tabPanel("Month and Week",
             sidebarLayout(
               sidebarPanel(
                 sliderInput("month_select_week", "Select Month", min = 1, max = 12, value = c(1, 12), step = 1),
                 sliderInput("week_select", "Select Week", min = 1, max = 5, value = c(1, 5), step = 1)
               ),
               mainPanel(
                 plotOutput("plot_month_week")
               )
             )
    )
  )
...
server <- function(input, output) {
  ...
  filtered_result_month_day <- reactive({
    result[result$Month >= input$month_select[1] & result$Month <= input$month_select[2] &
             result$Day >= input$day_select[1] & result$Day <= input$day_select[2], ]
  })
  
  # Render Month and Day Heatmap
  output$plot_month_day <- renderPlot({
    p <- ggplot(filtered_result_month_day(), aes(x = Day, y = Month, fill = Total)) +
      geom_tile() +
      scale_fill_gradient(low = "white", high = "red") +
      labs(title = "Heatmap by Month and Day",
           x = "Day",
  ...
  filtered_result_month_week <- reactive({
    result[result$Month >= input$month_select_week[1] & result$Month <= input$month_select_week[2] &
             result$Week >= input$week_select[1] & result$Week <= input$week_select[2], ]
  })
  
  # Render Month and Week Heatmap
  output$plot_month_week <- renderPlot({
    p <- ggplot(filtered_result_month_week(), aes(x = Month, y = Week, fill = Total)) +
      geom_tile() +
      scale_fill_gradient(low = "white", high = "steelblue") +
      labs(title = "Heatmap by Month and Week",
           x = "Month",
