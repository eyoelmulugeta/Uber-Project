...
# Create sample data
result <- data.frame(
  Base = c("Base1", "Base2", "Base3", "Base4", "Base5"),
  DayOfWeek = c("Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"),
  Total = sample(1:100, size = 35, replace = TRUE)
)
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("base_select", "Select Base", choices = unique(result$Base)),
...
server <- function(input, output) {
  ...
  # Filter data based on selected base
  filtered_result <- reactive({
    result %>%
      filter(Base == input$base_select)
  })
  
  # Render Heatmap
  output$plot <- renderPlot({
    p <- ggplot(filtered_result(), aes(DayOfWeek, Base, fill = Total)) +
      geom_tile(color = "white") +
      scale_fill_gradient(low = "white", high = "red") +
      labs(title = "Heatmap by Bases and Day of Week",
           x = "Day of Week",
           y = "Base") +
      theme_minimal()
    print(p)
