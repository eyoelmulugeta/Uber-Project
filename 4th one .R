...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      sliderInput("day_range", "Select Day Range", min = 1, max = 31, value = c(1, 31))
    ),
    mainPanel(
      plotOutput("plot")
    )
...
server <- function(input, output) {
  ...
  filtered_result <- reactive({
    result[result$Day >= input$day_range[1] & result$Day <= input$day_range[2], ]
  })
  
  # Render Plot
  output$plot <- renderPlot({
    p <- ggplot(filtered_result(), aes(Day, Total)) +
      geom_bar(stat = "identity", width = 0.7, fill = "steelblue") +
      labs(x = "Day of Month", y = "Trips") +
      ggtitle("Trips by Day of Month") +
      theme_minimal() +
